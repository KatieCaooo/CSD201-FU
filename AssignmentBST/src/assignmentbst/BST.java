...
public class BST<T extends Comparable<? super T>> {
    ...
    protected int countNode(BSTNode<T> p) {
        if (p != null) {
    ...
    protected void inorder(BSTNode<T> p, T value) {
        if (p != null) {
            inorder(p.left, value);
            if (p.el.compareTo(value) < 0) {
                visit(p);
            }
            inorder(p.right, value);
        }
    }
    ...
    public void iterativePreorder() {
        BSTNode<T> p = root;
        Stack<BSTNode<T>> travStack = new Stack<>();
        if (p != null) {
            travStack.push(p);
    ...
    public void MorrisPostorder() {
        BSTNode<T> p = new BSTNode<>(), tmp, q, r, s;
        p.left = root;
        while (p != null) {
            if (p.left == null) {
                p = p.right;
